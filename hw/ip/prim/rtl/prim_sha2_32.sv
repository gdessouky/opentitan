...
#(
      ...
      if (sha_en_i && fifo_rvalid_i) begin // valid incoming word part and SHA engine is enabled
        if (word_part_count == 2'b00) begin
          if (digest_mode_flag != SHA2_256) begin
            ...
            word_part_inc             = 1'b1;
            fifo_rready_o             = 1'b1;
            if (hash_process_i || process_flag) begin // ready to push out word (partial)
              word_valid      = 1'b1;
            ...
            if (hash_process_i || process_flag) begin
                sha_process = 1'b1;
            end
            if (sha_ready == 1'b1) begin
              // if word has been absorbed into hash engine
        ...
        full_word = word_buffer_q;
        if (word_part_count == 2'b00 && (hash_process_i || process_flag)) begin
          sha_process = 1'b1; // wait on hash_process_i
        end else if (word_part_count == 2'b01 && (hash_process_i || process_flag)) begin
          ...
          word_valid = 1'b0;
        end else if (word_part_count == 2'b10 && (hash_process_i || process_flag)) begin
  ...
  always_comb begin
    if (MultimodeEn) begin
      ...
      if ((word_part_reset || hash_start_i || !sha_en_i)) word_part_count_d = '0;
