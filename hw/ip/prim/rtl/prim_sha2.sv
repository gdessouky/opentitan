...
#(
  ...
  localparam int unsigned RnddWidth256 = $clog2(NumRound256),
  ...
  output sha_word64_t [7:0] digest, // tie off unused part of port when
  ...
  // compute hash
  always_comb begin : compress_round
    if (MultimodeEn) begin : compress_multimode
      hash_d = hash_q;
      if (wipe_secret) begin
        for (int i = 0; i < 8; i++) begin
          hash_d[i] = hash_q[i] ^ wipe_v;
        end
    ...
    end else begin  : compress_256
       // ~MultimodeEn
       hash256_d = hash256_q;
      if (wipe_secret) begin
        for (int i = 0; i < 8; i++) begin
          hash256_d[i] = hash256_q[i] ^ wipe_v[31:0];
        end
      end else if (init_hash) begin
  ...
  always_comb begin : compute_digest
    if (MultimodeEn) begin  : compute_digest_multimode
      ...
      if (wipe_secret) begin
        for (int i = 0 ; i < 8 ; i++) begin
          digest256_d[i] = digest256_q[i] ^ wipe_v[31:0];
        end
      end else if (hash_start) begin
        for (int i = 0 ; i < 8 ; i++) begin
  ...
  if (MultimodeEn) begin : gen_digest_multi
    assign digest = digest_q;
  end else begin  : gen_digest_256
    for (genvar i = 0; i < 8; i++) begin  : gen_assign_digest_256
      assign digest[i][31:0] = digest256_q[i];
      assign digest[i][63:32] = 32'b0;
    end
  end
  ...
  // compute w_index
  assign w_index_d = (~sha_en || hash_start)  ? '0            :  // clear
  ...
  always_comb begin
    ...
    if (!sha_en || hash_start) sha_st_d  = ShaIdle;
  end
