  ...
  always_comb begin : assign_digest_reg
    ...
    for (int i = 0; i < 8; i++) begin
      ...
      if (digest_size == SHA2_256) begin
        ...
        // digest SW -> HW
        digest_sw[i][63:32]    = conv_endian32(reg2hw.digest[2*i].q, digest_swap);
        digest_sw[i][31:0]     = conv_endian32(reg2hw.digest[2*i+1].q, digest_swap);
      ...
      if (digest_size_started_q == SHA2_256) begin
        ...
        // digest HW -> SW
        // digest swap the entire 64-bit word
        inter_digest           = conv_endian64(digest[i], digest_swap);
        hw2reg.digest[2*i].d   = inter_digest[63:32];
        hw2reg.digest[2*i+1].d = inter_digest[31:0];
